...
// ─── Image upload slot — uploads to Firebase Storage + saves to Assets ────────
const ImageUploadSlot = ({
  url, onUrlChange, tenantId, storage, db, assetType = 'room-image',
}: {
  url: string; onUrlChange: (v: string) => void;
  tenantId: string; storage: any; db: any; assetType?: string;
}) => {
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const storagePath = `assets/${tenantId}/${assetType}/${fileName}`;
  ...
  const displayUrl = !url || url.startsWith('__') ? '' : url;

  return (
    <div className="space-y-2">
      <Input value={displayUrl} onChange={e => onUrlChange(e.target.value)}
        placeholder="Paste image URL or upload below" className="bg-gray-50 text-xs h-8" />
...
// ─── Previews (unchanged from original) ──────────────────────────────────────
...
const RoomsPreview = ({ cfg }: { cfg: typeof DEFAULT_ROOMS }) => (
  <div className="rounded-xl overflow-hidden border border-slate-200 shadow-sm p-4"
    ...
    {cfg.rooms.length === 0 ? (
      <p className="text-xs text-slate-400 text-center py-4">No room cards yet.</p>
    ) : (
      <div className="grid gap-1.5"
        style={{ gridTemplateColumns: `repeat(${Math.min(cfg.rooms.length, 6)}, 1fr)` }}>
  ...
  <div className="rounded-xl overflow-hidden border border-slate-200 shadow-sm">
    <div className="flex items-center justify-between px-4 py-3" style={{ backgroundColor: cfg?.background || '#ffffff' }}>
      <div className="flex items-center gap-2">
        ...
        <span className="font-bold text-sm" style={{ color: primaryColor }}>Store</span>
      </div>
      ...
      <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white"
        style={{ background: primaryColor }}>Cart</div>
    </div>
    {cfg?.sticky && (
      <div className="text-[10px] text-emerald-600 bg-emerald-50 px-4 py-1 border-t border-emerald-100">
    ...
    <div className="px-4 py-4" style={{ backgroundColor: footerCfg?.backgroundColor || '#1e293b' }}>
      <div className="flex justify-between gap-3 mb-3">
        <div>
          <div className="flex items-center gap-1.5 mb-1.5">
            ...
            <span className="font-bold text-sm text-white">Store</span>
          </div>
        ...
        <div className="flex gap-5">
          {['Products', 'About', 'Contact'].map(l => (
            <div key={l}>
              <p className="text-xs font-semibold text-white mb-1">{l}</p>
              {['Link 1', 'Link 2'].map(s => <p key={s} className="text-[10px] text-white/40">{s}</p>)}
            </div>
          ))}
        </div>
      </div>
      <div className="border-t border-white/10 pt-2">
        <p className="text-[10px] text-white/40">
          {footerCfg?.copyrightText || '© 2024. All rights reserved.'}
        </p>
      </div>
    </div>
  </div>
...
const ThemePreview = ({ themeCfg }: any) => {
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-slate-200 shadow-sm p-4 space-y-3">
      <div>
        <p className="text-[10px] text-slate-400 mb-1">Typography</p>
      ...
      <div>
        <p className="text-[10px] text-slate-400 mb-1">Buttons</p>
        <div className="flex gap-2">
      ...
      <div>
        ...
        <div className="rounded-xl p-3 border text-xs" style={{ backgroundColor: cardBg, borderColor: cardBorder }}>
          ...
          <p style={{ color: body }}>Card body content.</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-3 gap-3">
        {['Eco Spray', 'Floor Cleaner', 'Dish Soap'].map((name, i) => (
          <div key={i} className="border overflow-hidden"
            style={{ backgroundColor: bg, borderColor: border, borderRadius: `${radius}px` }}>
            <div className="h-24 bg-gradient-to-br from-slate-200 to-slate-300 relative">
              {showTags && (
                <span className="absolute top-2 left-2 text-[8px] px-1.5 py-0.5 rounded-full font-medium"
                  style={{ background: tagBg, color: tagText }}>New</span>
              )}
            </div>
            <div className="p-2">
              ...
              <div className="mt-1.5 text-[9px] font-semibold text-center py-1 rounded-lg"
                style={{ background: btnBg, color: btnText }}>Add to Cart</div>
            </div>
          </div>
        ))}
      </div>
  ...
  <div className="rounded-xl overflow-hidden border border-slate-200 shadow-sm p-4 space-y-3">
    <div className="flex gap-3">
      ...
      <div className="min-w-0">
        <p className="font-bold text-sm truncate" style={{ color: primaryColor }}>
          {cfg?.content?.storyTitle || 'Our Story'}
        </p>
        <p className="text-xs text-slate-500 line-clamp-3 mt-1">
          {cfg?.content?.storyParagraph1 || 'Founded with a passion for eco-friendly cleaning...'}
        </p>
      </div>
    </div>
  </div>
  ...
  <div className="rounded-xl overflow-hidden border border-slate-200 shadow-sm p-4">
    <div className="space-y-2">
      {[['Eco Spray x1', 'R129.99'], ['Floor Cleaner x2', 'R239.98']].map(([name, price]) => (
        <div key={name} className="flex justify-between text-xs py-1.5">
      ...
      <div className="flex justify-between font-bold text-sm pt-1">
        <span>Total</span>
        <span style={{ color: primaryColor }}>R369.97</span>
      </div>
      <div className="flex gap-2 pt-1 flex-wrap">
        {cfg?.allowGuestCheckout && (
          <span className="text-[10px] bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-full">Guest checkout ✓</span>
        )}
        {cfg?.allowPromotionCodes && (
          <span className="text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full">Promo codes ✓</span>
        )}
      </div>
    </div>
  </div>
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {cfg?.EnablePaypal && (
        <div className="flex items-center gap-1.5 px-3 py-2 bg-yellow-50 border border-yellow-200 rounded-lg">
          ...
          <span className="text-xs font-medium text-yellow-700">PayPal</span>
        </div>
      )}
      {!cfg?.EnablePaystack && !cfg?.EnablePaypal && (
        <p className="text-xs text-slate-400">No payment methods enabled yet.</p>
      )}
    </div>
  </div>
);
  ...
  <div className="rounded-xl overflow-hidden border border-slate-200 shadow-sm p-4 space-y-2">
    {cfg?.contactInfo?.phone && (
      <div className="flex items-center gap-2 text-xs">
        ...
        <span className="text-slate-700">{cfg.contactInfo.phone}</span>
      </div>
    )}
    {cfg?.contactInfo?.email && (
      <div className="flex items-center gap-2 text-xs">
        <Mail className="w-3.5 h-3.5 text-emerald-500" />
        <span className="text-slate-700">{cfg.contactInfo.email}</span>
      </div>
    )}
    {!cfg?.contactInfo?.phone && !cfg?.contactInfo?.email && (
    ...
    <div className="flex flex-wrap gap-3">
      {cfg?.storeName && (
        <div className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
          ...
          <p className="text-sm font-semibold text-slate-700">{cfg.storeName}</p>
        </div>
      )}
      {cfg?.currency && (
        <div className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
          <p className="text-[10px] text-slate-400">Currency</p>
          <p className="text-sm font-semibold text-slate-700">{cfg.currency}</p>
        </div>
      )}
      {!cfg?.storeName && !cfg?.currency && (
        <p className="text-xs text-slate-400">No settings configured yet.</p>
      )}
    </div>
  </div>
);
...
export default function Features() {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          setConfig(data);
          if (data?.HomeCustomization?.featuresSection)
            setFeaturesConfig({ ...DEFAULT_FEATURES, ...data.HomeCustomization.featuresSection });
          if (data?.HomeCustomization?.roomsSection?.rooms?.length > 0)
            setRoomsConfig({ ...DEFAULT_ROOMS, ...data.HomeCustomization.roomsSection });
        }
        await fetchProductCategories(tenantId);
      } catch (err) {
        console.error('Features load error:', err);
        toast({ title: 'Could not load site config', variant: 'destructive' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const fetchProductCategories = async (tid = tenantId) => {
    if (!db || !tid) return [];
    setFetchingCategories(true);
    try {
      // Scope products to this tenant
      const q = query(collection(db, 'Products'), where('TenantId', '==', tid));
  ...
  const autoPopulateRooms = (cats: { id: string; label: string; count: number }[]) => {
    if (!cats.length) return;
    const populated = cats.map(cat => ({
      id: uid(), name: cat.label, productCount: cat.count, categoryId: cat.id, imageUrl: '',
    }));
    setRoomsConfig(prev => ({ ...prev, rooms: populated }));
    toast({ title: ` Auto-populated ${populated.length} room cards` });
  };
  ...
  const handleSave = async () => {
    if (!db || !tenantId) {
      toast({ title: 'No active tenant selected', variant: 'destructive' });
      return;
    }
    setSaving(true);
    try {
      ...
      const merged = {
        ...
        HomeCustomization: {
          ...
          roomsSection: { ...roomsConfig, rooms: updatedRooms },
        },
      };

      // Write to Sites/{tenantId}
      await setDoc(doc(db, 'Sites', tenantId), merged, { merge: true });

      setConfig(merged);
      toast({ title: ' Saved', description: `Site config updated for ${activeTenant?.Name}.` });
    } catch (err: any) {
  ...
  const handleLogoUpload = async (e: any) => {
    ...
    try {
      ...
      const fileName = `${Date.now()}_logo_${file.name}`;
      const storagePath = `assets/${tenantId}/logo/${fileName}`;
      const storageRef = ref(storage, storagePath);
      const task = uploadBytesResumable(storageRef, compressed);
      await new Promise<void>((resolve, reject) => {
        task.on('state_changed', () => {}, reject, async () => {
          ...
          await addDoc(collection(db, 'Assets'), {
            TenantId: tenantId, URL: url, Type: 'logo',
            FileName: fileName, StoragePath: storagePath, CreatedAt: serverTimestamp(),
          });
          handleChange('NavbarCustomization.logoURL', url);
          resolve();
        });
      });
  ...
  const addFeatureItem = () =>
    setFeaturesConfig(p => ({ ...p, items: [...p.items, { id: uid(), icon: 'Leaf', title: 'New Feature', description: 'Description here' }] }));
  const removeFeatureItem = (id: string) =>
    setFeaturesConfig(p => ({ ...p, items: p.items.filter(i => i.id !== id) }));
  const updateFeatureItem = (id: string, key: string, val: string) =>
    setFeaturesConfig(p => ({ ...p, items: p.items.map(i => i.id === id ? { ...i, [key]: val } : i) }));

  const addRoom = () =>
    setRoomsConfig(p => ({ ...p, rooms: [...p.rooms, { id: uid(), name: 'New Room', productCount: 0, categoryId: '', imageUrl: '' }] }));
  const removeRoom = (id: string) =>
    setRoomsConfig(p => ({ ...p, rooms: p.rooms.filter(r => r.id !== id) }));
  ...
  const handleRoomCategorySelect = (roomId: string, categoryId: string) => {
    ...
    setRoomsConfig(p => ({
      ...
      rooms: p.rooms.map(r => r.id === roomId
        ? { ...r, categoryId, name: cat ? cat.label : r.name, productCount: cat ? cat.count : r.productCount }
        : r),
    }));
  };

  const primaryColor = config.ThemeCustomization?.buttons?.primaryBg
    || config.ThemeCustomization?.primaryColor || '#00e676';
    ...
    return (
      <div className="flex items-center justify-center min-h-[40vh]">
        <div className="text-center space-y-3 max-w-sm">
          ...
          <p className="text-sm text-slate-500">Select a tenant from the sidebar to configure its site.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-[40vh]">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500">Loading {activeTenant?.Name} config…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 pb-12">
      {/* Sticky header */}
      <div className="flex justify-between items-center sticky top-0 z-30 bg-white/95 backdrop-blur py-3 px-1 -mx-1">
        <div>
          <h2 className="text-lg font-bold text-slate-800">Store Configuration</h2>
          <p className="text-xs text-slate-500">
            Editing: <span className="font-semibold text-emerald-600">{activeTenant?.Name}</span>
      ...
      <Section title="Features Section" icon={Sparkles} preview={<FeaturesPreview cfg={featuresConfig} />}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5"><Label className="text-xs text-slate-500">Badge Text</Label>
            <Input value={featuresConfig.badgeText} onChange={e => setFeaturesConfig(p => ({ ...p, badgeText: e.target.value }))} className="bg-white" /></div>
          <div className="space-y-1.5"><Label className="text-xs text-slate-500">Main Heading</Label>
            <Input value={featuresConfig.heading} onChange={e => setFeaturesConfig(p => ({ ...p, heading: e.target.value }))} className="bg-white" /></div>
          <div className="space-y-1.5"><Label className="text-xs text-slate-500">Highlighted Part</Label>
            <Input value={featuresConfig.headingHighlight} onChange={e => setFeaturesConfig(p => ({ ...p, headingHighlight: e.target.value }))} className="bg-white" /></div>
          <div className="space-y-1.5"><Label className="text-xs text-slate-500">Subheading</Label>
            <Input value={featuresConfig.subheading} onChange={e => setFeaturesConfig(p => ({ ...p, subheading: e.target.value }))} className="bg-white" /></div>
        </div>
        ...
        <div className="border-t pt-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" onClick={addFeatureItem} className="gap-1.5">
          ...
          {featuresConfig.items.map((item, idx) => (
            <div key={item.id} className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => removeFeatureItem(item.id)} className="text-red-400 hover:text-red-600">
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="space-y-1.5"><Label className="text-xs text-slate-500">Title</Label>
                  <Input value={item.title} onChange={e => updateFeatureItem(item.id, 'title', e.target.value)} className="bg-gray-50" /></div>
                <div className="space-y-1.5"><Label className="text-xs text-slate-500">Description</Label>
                  <Input value={item.description} onChange={e => updateFeatureItem(item.id, 'description', e.target.value)} className="bg-gray-50" /></div>
                <div className="space-y-1.5">
                    ...
                    {ICON_OPTIONS.map(({ value, Icon: Ico }) => (
                      <button key={value} type="button" onClick={() => updateFeatureItem(item.id, 'icon', value)}
                        className={`p-1.5 rounded-lg transition-all flex items-center justify-center
                          ${item.icon === value ? 'bg-emerald-100 ring-2 ring-emerald-500' : 'hover:bg-gray-200'}`}>
      ...
      <Section title="Shop by Room / Category" icon={HomeIcon} preview={<RoomsPreview cfg={roomsConfig} />}>
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-emerald-800">Product Categories</p>
              {productCategories.length > 0 ? (
                ...
                <p className="text-xs text-emerald-600 mt-1">No categories found. Click Sync.</p>
              )}
            </div>
            <div className="flex gap-2 flex-shrink-0">
              <Button size="sm" variant="outline" onClick={() => fetchProductCategories()}
                disabled={fetchingCategories} className="gap-1.5 border-emerald-300 text-emerald-700">
                {fetchingCategories ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <RefreshCw className="w-3.5 h-3.5" />}Sync
              </Button>
              <Button size="sm" onClick={() => fetchProductCategories().then(cats => { if (cats?.length) autoPopulateRooms(cats); })}
                disabled={fetchingCategories} className="gap-1.5 bg-emerald-500 hover:bg-emerald-600">
                <RefreshCw className="w-3.5 h-3.5" />Re-populate
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5"><Label className="text-xs text-slate-500">Section Heading</Label>
            <Input value={roomsConfig.heading} onChange={e => setRoomsConfig(p => ({ ...p, heading: e.target.value }))} className="bg-white" /></div>
        ...
        <div className="border-t pt-4 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" onClick={addRoom} className="gap-1.5">
            ...
            <div key={room.id} className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  {productCategories.length > 0 ? (
                    <select className="w-full border border-gray-200 rounded-lg px-3 py-2 bg-gray-50 text-sm"
                    ...
                    <Input value={room.categoryId} onChange={e => updateRoom(room.id, 'categoryId', e.target.value.toLowerCase())}
                      placeholder="e.g. kitchen" className="bg-gray-50 font-mono text-sm" />
                  )}
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs text-slate-500">Product Count</Label>
                  <Input type="number" value={room.productCount}
                    onChange={e => updateRoom(room.id, 'productCount', Number(e.target.value))} className="bg-gray-50" />
                </div>
              </div>
              <div className="space-y-1.5">
      ...
      <Section title="Navbar" icon={Navigation} preview={<NavbarPreview cfg={config.NavbarCustomization} primaryColor={primaryColor} />}>
        <div className="space-y-4">
          <div><Label>Logo URL</Label>
            <Input value={config.NavbarCustomization?.logoURL || ''} onChange={e => handleChange('NavbarCustomization.logoURL', e.target.value)} /></div>
          <div><Label>Upload Logo</Label>
            <Input type="file" accept="image/*" onChange={handleLogoUpload} />
            {config.NavbarCustomization?.logoURL && (
              <img src={config.NavbarCustomization.logoURL} alt="Logo" className="mt-2 h-16 object-contain" />
            )}
          </div>
          ...
          <div className="flex items-center justify-between">
            <Label>Keep Navbar Sticky</Label>
            <Checkbox checked={config.NavbarCustomization?.sticky || false} onCheckedChange={v => handleChange('NavbarCustomization.sticky', v)} />
          </div>
        </div>
      </Section>
      ...
      <Section title="Footer" icon={Layers} preview={<FooterPreview footerCfg={config.FooterCustomization} primaryColor={primaryColor} />}>
        <div className="space-y-4">
          ...
          <div><Label>Copyright Text</Label>
            <Textarea value={config.FooterCustomization?.copyrightText || ''} onChange={e => handleChange('FooterCustomization.copyrightText', e.target.value)} /></div>
          <ColorRow label="Primary Color" value={config.ThemeCustomization?.primaryColor || '#000000'} onChange={(v: string) => handleChange('ThemeCustomization.primaryColor', v)} />
      ...
      <Section title="Global Theme" icon={Palette} preview={<ThemePreview themeCfg={config.ThemeCustomization} />}>
        <div className="space-y-4">
          <div><Label>Font</Label>
            <select className="border rounded p-2 w-full mt-1" value={config.ThemeCustomization?.fontFamily || ''}
              onChange={e => handleChange('ThemeCustomization.fontFamily', e.target.value)}>
              <option value="">Select font</option>
              {['Inter', 'Poppins', 'Roboto', 'Lato', 'Montserrat', 'Open Sans'].map(f =>
      ...
      <Section title="Product Card" icon={Package} preview={<ProductCardPreview cfg={config.ProductCardCustomization} primaryColor={primaryColor} />}>
        <div className="space-y-4">
          ...
          <div><Label>Corner Radius: {config.ProductCardCustomization?.appearance?.borderRadius || 16}px</Label>
            <Input type="range" min="0" max="32" step="2"
              value={config.ProductCardCustomization?.appearance?.borderRadius || 16}
              onChange={e => handleChange('ProductCardCustomization.appearance.borderRadius', e.target.value)} /></div>
          <div className="flex items-center justify-between"><Label>Show Product Tags</Label>
            <Checkbox checked={config.ProductCardCustomization?.display?.showTags !== false}
              onCheckedChange={v => handleChange('ProductCardCustomization.display.showTags', v)} /></div>
          <div className="flex items-center justify-between"><Label>Show Description</Label>
            <Checkbox checked={config.ProductCardCustomization?.display?.showDescription !== false}
              onCheckedChange={v => handleChange('ProductCardCustomization.display.showDescription', v)} /></div>
          <p className="font-semibold text-sm pt-2">Colors</p>
      ...
      <Section title="About Page" icon={Type} preview={<AboutPreview cfg={config.AboutCustomization} primaryColor={primaryColor} />}>
        <div className="space-y-4">
          <div><Label>Story Title</Label>
            <Input value={config.AboutCustomization?.content?.storyTitle || ''} onChange={e => handleChange('AboutCustomization.content.storyTitle', e.target.value)} /></div>
          <div><Label>Story Paragraph 1</Label>
            <Textarea rows={3} value={config.AboutCustomization?.content?.storyParagraph1 || ''} onChange={e => handleChange('AboutCustomization.content.storyParagraph1', e.target.value)} /></div>
          <div><Label>Story Paragraph 2</Label>
            <Textarea rows={3} value={config.AboutCustomization?.content?.storyParagraph2 || ''} onChange={e => handleChange('AboutCustomization.content.storyParagraph2', e.target.value)} /></div>
          {[1, 2, 3].map(n => (
            <div key={n} className="bg-white/70 p-4 rounded-lg space-y-2 border border-gray-100">
              <p className="font-medium text-sm">Value {n}</p>
              <Input placeholder="Title" value={(config.AboutCustomization?.content as any)?.[`value${n}Title`] || ''}
                onChange={e => handleChange(`AboutCustomization.content.value${n}Title`, e.target.value)} />
              <Textarea rows={2} placeholder="Description" value={(config.AboutCustomization?.content as any)?.[`value${n}Desc`] || ''}
                onChange={e => handleChange(`AboutCustomization.content.value${n}Desc`, e.target.value)} />
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section title="Cart" icon={ShoppingCart} preview={<CartPreview cfg={config.CartCustomization} primaryColor={primaryColor} />}>
        <div className="space-y-4">
          <div><Label>Tax (%)</Label>
            <Input value={config.CartCustomization?.TaxAmount || ''} onChange={e => handleChange('CartCustomization.TaxAmount', e.target.value)} /></div>
          <div className="flex items-center justify-between"><Label>Allow Guest Checkout</Label>
            <Checkbox checked={config.CartCustomization?.allowGuestCheckout || false} onCheckedChange={v => handleChange('CartCustomization.allowGuestCheckout', v)} /></div>
          <div className="flex items-center justify-between"><Label>Allow Promotion Codes</Label>
            <Checkbox checked={config.CartCustomization?.allowPromotionCodes || false} onCheckedChange={v => handleChange('CartCustomization.allowPromotionCodes', v)} /></div>
        </div>
      </Section>
      ...
      <Section title="Checkout" icon={CreditCard} preview={<CheckoutPreview cfg={config.CheckoutCustomization} />}>
        <div className="space-y-4">
          <div className="flex items-center justify-between"><Label>Enable Paystack</Label>
            <Checkbox checked={config.CheckoutCustomization?.EnablePaystack || false} onCheckedChange={v => handleChange('CheckoutCustomization.EnablePaystack', v)} /></div>
          <div className="flex items-center justify-between"><Label>Enable PayPal</Label>
            <Checkbox checked={config.CheckoutCustomization?.EnablePaypal || false} onCheckedChange={v => handleChange('CheckoutCustomization.EnablePaypal', v)} /></div>
          <div className="flex items-center justify-between"><Label>Enable Third Party Payment</Label>
            <Checkbox checked={config.CheckoutCustomization?.EnableThirdPartyPayment || false} onCheckedChange={v => handleChange('CheckoutCustomization.EnableThirdPartyPayment', v)} /></div>
        </div>
      </Section>
      ...
      <Section title="Contact" icon={MessageSquare} preview={<ContactPreview cfg={config.ContactCustomization} />}>
        <div className="space-y-4">
          <div><Label>Phone</Label>
            <Input value={config.ContactCustomization?.contactInfo?.phone || ''} onChange={e => handleChange('ContactCustomization.contactInfo.phone', e.target.value)} /></div>
          <div><Label>Email</Label>
            <Input value={config.ContactCustomization?.contactInfo?.email || ''} onChange={e => handleChange('ContactCustomization.contactInfo.email', e.target.value)} /></div>
          <div><Label>Address</Label>
            <Textarea value={config.ContactCustomization?.contactInfo?.Address || ''} onChange={e => handleChange('ContactCustomization.contactInfo.Address', e.target.value)} /></div>
        </div>
      </Section>
      ...
      <Section title="General Settings" icon={Settings} preview={<GeneralPreview cfg={config.GeneralSettings} />}>
        <div className="space-y-4">
          <div><Label>Store Name</Label>
            <Input value={config.GeneralSettings?.storeName || ''} onChange={e => handleChange('GeneralSettings.storeName', e.target.value)} /></div>
          <div><Label>Currency</Label>
            <select className="border rounded p-2 w-full mt-1" value={config.GeneralSettings?.currency || ''}
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-3">
            ...
            <code className="text-xs text-blue-800 font-mono">Sites/{tenantId}</code>
          </div>
        </div>
      </Section>
    </div>
